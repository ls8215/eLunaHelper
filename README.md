...
2. **Why use this extension?** To some extent, it improves terminology accuracy, since both the active segment and its term pairs are supplied to the translation service, which can incorporate the provided terms into the translation. It also enhances efficiency—you no longer need to copy and paste text into separate AI chatbots or online translation interfaces.
3. **It has limitations.** The quality of the translation depends on the selected service and several factors, such as the model or engine used, your prompt (for LLMs), and the source text itself. Therefore, a consistently accurate translation cannot be guaranteed. For LLM-based services, different combinations of these factors may produce different results, and there is no one-size-fits-all solution in terms of model selection or prompt design. A known limitation is that, because the segment and its term pairs are supplied out of context, the translation engine may occasionally choose an inappropriate term.
4. **Cost and usage.** Some services (such as DeepSeek, OpenAI, or DeepL Pro) require paid API keys. Charges are typically based on usage, measured in tokens or characters, depending on the provider. Both input (including your prompt) and output are billed. For instance, DeepSeek charges ¥2 per million input tokens and ¥3 per million output tokens. Roughly estimated, a typical day’s translation workload may cost around ¥0.1 under DeepSeek’s pricing, though the actual amount will vary depending on usage. Google Translate (free tier) and DeepL Free are also supported, but they may have rate limits or quality differences compared with paid versions.
...
2. Click the puzzle-piece icon to open the list of installed extensions, and you should see the eLuna Helper extension
3. Click the extension, you should see a list of four inactive services
4. Click the the gear icon to configure the services
...
5. `Temperature`: You can tweak this figure to see if the result meets your expection. The lower this figure is, the less random and creative the result will be. For translation, a figure between 1 to 1.3 will be a reasonable. 
